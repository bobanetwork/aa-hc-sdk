...
contract HCHelper {
    ...
    function PutResponse(bytes32 subKey, bytes calldata response) public {
        require(
            RegisteredCallers[msg.sender].owner != address(0),
            "Unregistered caller"
        );
        require(response.length >= 32 * 4, "Response too short");
        (, , uint32 errCode, ) = abi.decode(
            response,
    ...
    function PutSysResponse(bytes32 subKey, bytes calldata response) public {
        require(
            msg.sender == systemAccount,
            "Only systemAccount may call PutSysResponse"
        );
        require(response.length >= 32 * 4, "Response too short");
        (, , uint32 errCode, ) = abi.decode(
            response,
        ...
        require(
            errCode >= 2,
            "PutSysResponse() may only be used for error responses"
        );
    ...
    function RemoveResponses(bytes32[] calldata mapKeys) public {
        require(
            msg.sender == systemAccount,
            "Only systemAccount may call RemoveResponses"
        );
    ...
    function getEntry(
        bytes32 mapKey
    ) internal returns (bool, uint32, bytes memory) {
        bytes memory entry;
        bool found;
        uint32 errCode;
        ...
        if (entry.length == 1) {
            ...
            found = true;
            (srcAddr, srcNonce, errCode, response) = abi.decode(
                entry,
                (address, uint256, uint32, bytes)
            );
    ...
    function TryCallOffchain(
        bytes32 userKey,
        bytes memory req
    ) public returns (uint32, bytes memory) {
        bool found;
        uint32 errCode;
        bytes memory ret;
        require(
        ...
        if (found) {
            ...
            if (found) {
                ...
                bytes memory r2 = bytes.concat(
                    prefix,
                    abi.encodePacked(msg.sender, userKey, req)
                );
                assembly {
